...
#Read in PDFs
rootdir = "P:/2020/14/Tingsrätter/Alingsås/Domar/testcases"
output_path = "P:/2020/14/Tingsrätter/Alingsås/Domar/testcases/registerout.csv"
txtpath = 'P:/2020/14/Tingsrätter/Alingsås/Domar/testcases/rulingpaths.txt'
pdf_files = []
unreadable_files = []
exclude = set(["Case_Sorting_SC"
               ])

includes = 'testcases'  #change back to all cases to loop over all files
...
for file in pdf_files:
    try:
        ...
        data = {'caseid':[], 'type':[], 'casetype':[], 'court':[], 'year':[], 'plaintiff':[], 'defendant':[], 'judge':[], 'judgetitle':[], 'filepath':[]}
        ...
        lastPageFormattedPrint = (pages_text_formatted[appendixPageNo-1]).split(".")
        
        if "ÖVERKLAG" not in lastPageFormatted:
            try:
                judgePageNo = ([i for i, item in enumerate(pages_text) if re.search("ÖVERKLAG", item)])[0]+1
                lastPageFormatted3 = '.'.join((pages_text_formatted[judgePageNo-1]).split("."))  
                lastPageFormatted4 = '.'.join((pages_text_formatted[judgePageNo-2]).split("."))  
                lastPageFormatted5 = '.'.join((pages_text_formatted[judgePageNo-3]).split("."))  
                lastPageFormatted6 = '.'.join((pages_text_formatted[judgePageNo-4]).split("."))  
                lastPageFormatted = lastPageFormatted3 + lastPageFormatted4+ lastPageFormatted5 + lastPageFormatted6
            except IndexError:
                lastPageFormatted = lastPageFormatted1
        ...
        #Document type
        if 'deldom' in filename or ' deldom ' in header or 'mellandom' in filename or 'mellandom' in header:
            docType = "deldom"
        ...
        try: 
            ...
            defNo = ''.join(searchKey(idNo, svarandeString, 2).split())
        except AttributeError:
        ...
        if docType == 'slutligt beslut' or docType == 'protokoll':
            ...
            judgeTitle = searchLoop(judgeSearchProtokoll, fullTextOG, 1)
        ...
        if docType == 'dom' or docType == 'deldom':
            ...
            #Write filepaths of custody and divorce rulings to textfile
        ...
        data = {'caseid':[], 'type':[], 'casetype':[], 'court':[], 'year':[], 'plaintiff':[], 'defendant':[], 'judge':[], 'judgetitle':[], 'filepath':[]}
        data['filepath'].append(file)  
        data['caseid'].append('error')
        data['type'].append('error')
        data['casetype'].append('error')
        data['court'].append('error')
        data['year'].append('error')
        data['plaintiff'].append('error')
        data['defendant'].append('error')
        data['judge'].append('error')
        data['judgetitle'].append('error')
        data['filepath'].append(file)
