...
if FULLSAMPLE == 1: #to create actual dataset
    ...
    ROOTDIR = "P:/2020/14/Kodning/Test-round-5-Anna/check/"
...
def word_classify(text, entity_req, strictness):
    ...
    print_output('NER clean_list', clean_list)
    ...
    print_output('NER grouped', grouped)
...
def get_party(parties, part_for_name, use_ner):
    ...
    plaint_lst = []
    parties = [clean_text(part, clean_partyname) for part in parties]
    parties = [x for x in parties if x if x != "\n"]
...
def get_ruling(fulltext_form):
    ...
    print_output("Term found for splitting ruling from end of text", second_ruling_splitter)
    ruling_form = re.split(second_ruling_splitter, ruling_form)[0]
    print_output("This ruling should be fully split:", ruling_form.split("delimit"))
...
def get_childname(ruling_og, plaint_name, defend_name, year):
    ...
    Note:
        - p_name = word_classify(relev, "PER",0.9) -> changed strictness to 0.9 bc of Attunda Scan_10_Jun_2022_at_13.56
    """
    ...
    if childid_name:
        ...
        for i in childid_name:
            ...
            print_output("Child name before NER",name)
            
            if len(name) == 1:
                names = name
            else:
                p_name = word_classify(relev, "PER",0.9)
                print_output("Child names",word_classify(relev, "PER",0.9))
