...
#Define Paths
pdf_dir = "P:/2020/14/Kodning/Test-round-2/files"
...
rejectKey = ['inte längre' , 'avslå', 'utan', 'ingen']
    ...
    if len(splitTextOG) > 1: 
        ...
        for i in childNoRes:   
            ...
            if childNameFirst is not None:
                actualFirstName = ('([A-ZÅÄÖ][a-zåäöé]+\s*)' + childNameFirst + '[,]?[)]?\s*' + i)
                childNameFirst1 = searchKey(actualFirstName, fullTextOG, 1)
                if childNameFirst1 is not None:
                    childNameFirst = childNameFirst1.lower()
            ...
            for key in umgangeKey:
                for term in rejectKey:
                    if childNameFirst == 'not found':
                        ...
                        print('umg1')
                        break
                    elif findTwoWords(key, childNameFirst, rulingOnly) and term not in findTwoWords(key, childNameFirst, rulingOnly):  
                        print('umg2')
                        dummyVisit = 1
                        break
            ...
            #N. Physical custody 
            if childNameFirst in findTwoWords('boende', plaintNameFirst, rulingOnly) :
                dummyPhys = 1
            ...
            for termAgree in agreementKey:
                print(termAgree)
                if svNameFirst in findTwoWords('yrkandet', term, fullText):
                    dummyAgree = 1
                    break
                for termHelper in agreementAdd:
                    print(termAgree + termHelper)
                    if findTwoWords(termAgree, termHelper, fullText) is not emptyString:
                        print('agree1')
                        dummyAgree = 1
                        break
                    else:
            ...
            #Main hearing 
            for key in mainHearingKey:
                for term in rejectKey:
                    if not findTwoWords(key, term, fullText):
                        print(findTwoWords(key, term, fullText))
                        print('mainhear1: ' + key)
                        dummyMainHear = 1
                        break
                    elif findTwoWords(key,term,fullText):
                        print('mainhear2: ' + key)
                        dummyMainHear = 0
                        break
                    else:
